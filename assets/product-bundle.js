class ProductBundle extends HTMLElement {
    ...
    onHandleCheckedProduct(event) {
        ...
        const includeText = event.currentTarget.getAttribute('data-bundle-include');
        const includedText = event.currentTarget.getAttribute('data-bundle-included');
    ...
    onHandleCloseProduct(event){
        ...
        const item = event.currentTarget.closest('.bundle-product-item');
        ...
        if (showBundleTotalPrice) {
            ...
            if(bundleItem.length > 0){
                bundleItem.forEach((item) => {
                    ...
                    if(selectElement) {
                        price = selectElement[selectElement.selectedIndex].getAttribute('data-price');
                    } else {
                        if (inputElement) {
                            price = inputElement.getAttribute('data-price');
                        } else {
                            price = item.querySelector('input[name=id]').getAttribute('data-price');
                        }
                    }
